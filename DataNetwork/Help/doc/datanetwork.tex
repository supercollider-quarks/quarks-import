...
Three methods of mapping data (see table \ref{oscinterfaceMB}):
\begin{description}
...
\verb|/map/minibee/output| & isii & port no., name, node ID, minibee ID & map node output to MiniBee output (reply /mapped/minibee)\\
...
\verb|/mapped/minibee| & ii & node ID, minibee ID & reply to /map/minibee messages \\
\hline
\end{tabular}
\end{center}
