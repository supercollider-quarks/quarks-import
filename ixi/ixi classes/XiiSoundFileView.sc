/*
// test the view
a = XiiSoundFileView.new(XQ.buffers(XQ.poolNames[0])[0], 0, XQ.poolNames[0], [0,0], nil)
...
XiiSoundFileView {	
	...
	var chNum, synth, loop, loopButt, playButt,setSelButt, deSelButt, resolutionSlider, zoomSlider, volSlider, cropButt;
...
//		if(caller.isKindOf(XiiBufferPool), { // if the caller is bufferpool
			win = GUI.window.new(filename+"  -  bufferPool: "+poolname, 							Rect(200, 600, 800, 238), 
...
//		});
					...
					if(loop == false, {
						if(chNum == 1, {
							synth = Synth(\xiiPlayBufXSndFileView1x1, 
										[\bufnum, buffer.bufnum, \start, start, \end, end, \vol, vol]);
						}, {
		...
		zoomSlider = GUI.slider2D.new(win, Rect(60,195,176, 35)) // scroll w/zoom x - felix!
					.canFocus_(false)
					.action_({|sl| 
						var y;
		...
		volSlider = OSCIISlider(win, Rect(348, 196, 90, 8), "vol", 0, 1, 1, 0.01)
					.canFocus_(false)
					.value_(1)
					.font_(GUI.font.new("Helvetica", 9))
					.action_({arg slider; 
		...
		GUI.popUpMenu.new(win, Rect(448, 196, 44, 16)) // outbusses
			.items_(
				if(chNum==2, {
		...
		deSelButt = GUI.button.new(win, Rect(500, 195, 60, 18))
			...
			.action_({
				...
				if(synthRunFlag, {	
					if(loop == true, {
						...
						if(chNum == 1, {
							synth = Synth(\xiiLoopBufXSndFileView1x1, 
								[\bufnum, buffer.bufnum, \start, 0, \end, soundfile.numFrames, \vol, vol]);
						}, {
							synth = Synth(\xiiLoopBufXSndFileView2x2, 
								[\bufnum, buffer.bufnum, \start, 0, \end, soundfile.numFrames, \vol, vol]);
						});
		...
		setSelButt = GUI.button.new(win, Rect(565, 195, 75, 18))
		...
		cropButt = GUI.button.new(win, Rect(645, 195, 45, 18))
			...
			.action_({arg butt; 
				...
				if(caller.isKindOf(XiiBufferPlayer), {
					...
					//caller.synthList[chnl].free;
					//caller.synthList[chnl] = nil;
				});
				
				{
				// crop the sound
				XQ.buffers(poolname.asSymbol)[fileNumInPool]
					.crop(startFrame, numSelFrames, true);
				...
				{
				soundfile = SoundFile.new;
				soundfile.openRead(XQ.buffers(poolname.asSymbol)[fileNumInPool].path);
				sndfileview
					.soundfile_(soundfile)
					.read(0, soundfile.numFrames);
				...
				soundfile.close;
				sndfileview.refresh; win.refresh;
				}.defer(0.5);
				*/
				
				/*
				var cropwin, bufbutt, filebutt;
				cropwin = GUI.window.new("- cropping -", 
								Rect(222, 500, 260, 120), resizable:false).front;
				GUI.staticText.new(cropwin, Rect(30, 0, 190, 50))
					.font_(GUI.font.new("Helvetica", 12))
					.string_("Attencion !!! ");
				GUI.staticText.new(cropwin, Rect(30, 20, 200, 50))
					.font_(GUI.font.new("Helvetica", 9))
					.string_("Do you want cropping to take effect on buffer only");
				GUI.staticText.new(cropwin, Rect(30, 35, 190, 50))
					.font_(GUI.font.new("Helvetica", 9))
					.string_("or buffer AND the soundfile as saved to disk?");

				bufbutt = GUI.button.new(cropwin, Rect(30, 75, 90, 20))
					.states_([["buffer only", Color.black, Color.clear]])
					.action_({
						var startFrame, numSelFrames;
						startFrame = sndfileview.selections[0][0]; // the start
						numSelFrames = sndfileview.selections[0][1]; // end of the selection
						XQ.buffers(poolname.asSymbol)[fileNumInPool]
				...
				filebutt = GUI.button.new(cropwin, Rect(130, 75, 90, 20))
					.states_([["buffer + file", Color.black, Color.clear]])
					.focus(true)
					.action_({
						var startFrame, numSelFrames;
						startFrame = sndfileview.selections[0][0]; // the start
		...
		numFramesView = GUI.staticText.new(win, Rect(503, 219, 80, 12))
					.font_(GUI.font.new("Helvetica", 9))
					.string_("frames : "+ selArray[1] );

		selStartView = GUI.staticText.new(win, Rect(580, 219, 80, 12))
					.font_(GUI.font.new("Helvetica", 9))
					.string_("selStart : "+ selArray[0] );

		selEndView = GUI.staticText.new(win, Rect(660, 219, 80, 12))
					.font_(GUI.font.new("Helvetica", 9))
				...
				if(sndfileview.selectionSize(0) < 10, {
 				...
 				if(butt.value == 1, {
					...
					if(loop == false, {
						if(chNum == 1, {
							synth = Synth(\xiiPlayBufXSndFileView1x1, 
								[\out, outbus, \bufnum, buffer.bufnum, \start, start, \end, end, \vol, vol]);
						}, {
							synth = Synth(\xiiPlayBufXSndFileView2x2, 
								[\out, outbus, \bufnum, buffer.bufnum, \start, start, \end, end, \vol, vol]);
						});
	...
	setBufferPlayerSelections {arg startPos, numSelectedFrames;
		...
		sndfileview
			//.setSelectionColor(1, Color(0.5,0.5,0.7))
			//.setSelectionColor(1, Color.blue)	
