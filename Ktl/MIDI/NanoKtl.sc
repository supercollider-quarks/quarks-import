...
NanoKtl : MIDIKtl { 
	...
	*initClass {
	...
	init { 
		...
		orderedKtlNames = ktlNames[1].keys.reject(_ == 'mode').asArray.sort.clump(9)[[2, 3, 1, 0]].flat.postcs;
	...
	mapToEnvirGui { |gui, scene, indices| 
		...
		elementKeys = orderedKtlNames[indices - 1].postcs; 
...
//		mastaFunc = Volume.softMasterVol(0.05, server, \midi.asSpec);
			...
			(1..4).do { |scene| this.mapS(scene, \sl9, 
...
//		mastaFunc = Volume.softMasterVol(0.05, server, \midi.asSpec);
	...
	*makeDefaults { 
		...
		// lookup for all scenes and ktlNames, \sl1, \kn1, \bu1, \bd1, 
