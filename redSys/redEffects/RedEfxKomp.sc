...
RedEfxKomp : RedEffectModule {
	*def {
		^SynthDef(\redEfxKomp, {|out= 0, mix= -1, preGain= 1, postGain= 0.25, thresh= 0.1, ratio= 5, atk= 0.05, rel= 0.05|
			...
			wet= dry*(1-cmp);
			ReplaceOut.ar(out, XFade2.ar(dry, wet, mix, postGain));
		}, metadata: (
			specs: (
				\out: \audiobus.asSpec,
				\mix: ControlSpec(-1, 1, 'lin', 0, -1),
				\preGain: ControlSpec(0, 4, 'lin', 0, 1),
				\postGain: ControlSpec(0, 4, 'lin', 0, 0.25),
